      ...
      {items.map((inv) => (
        <div key={inv.id} className="card p-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-400">{new Date(inv.event.startDate).toLocaleString()} — {new Date(inv.event.endDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
          </div>
